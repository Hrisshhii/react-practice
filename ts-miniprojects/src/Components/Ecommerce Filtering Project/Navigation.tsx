    ...
    <aside className={`flex flex-col transition-[width] duration-500 font-mono h-screen ${showNavBar?"bg-[#1A1C1E] w-64":"w-16 bg-[#1A1C1E] justify-between"}`}>
      <div className="px-3 pb-2 pt-6">
        ...
        <div className={`text-center pt-4`}>
          <MdOutlineArrowBackIos onClick={()=>setShowNavbar(!showNavBar)} size={24} 
            className={`text-white/80 hover:text-white/40 cursor-pointer 
      ...
      <div className={`flex-1 overflow-y-auto overflow-x-hidden scrollbar-hidden px-3 transition ${showNavBar?"":"hidden"}`}>
        <div className="flex flex-col gap-2 ">
          <input type="text" placeholder="Search Items . . . " className="text-[1.15rem] px-3 py-3 text-white/65 bg-transparent border-solid rounded-full focus:outline-none"/>
          <div className="grid grid-cols-2 gap-1.5">
            <input type="text" placeholder="Max" className="text-[1.1rem] px-3 py-1.5 text-white/65 bg-transparent border-solid rounded-lg focus:outline-none"/>
            <input type="text" placeholder="Min" className="text-[1.1rem] px-3 py-1.5 text-white/65 bg-transparent border-solid rounded-lg focus:outline-none"/>
          </div>
          <div>
            <h2 className="text-center text-[#bbc9ee]">Categories</h2>
          </div>
          {categories.map(cat => (
          ...
          <div>
            <h2 className="text-center text-[#bbc9ee]">Keywords</h2>
          </div>
          {keyword.map((word)=>(
          ...
          <button className="font-mono mt-3 text-sm text-semibold bg-black/50 hover:bg-black/10 rounded-md py-3 px-3 border-solid border-white/30 transition text-white cursor-pointer">Reset Filters</button>
        </div>
      </div>
